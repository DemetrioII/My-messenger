...
class TCPTransport : public ITransport {
	std::optional<std::vector<uint8_t>> extract_complete_message(int fd, std::vector<uint8_t>& buffer) const {
...
public:
	...
	ssize_t send(int fd, const std::vector<uint8_t>& data) const override {
		try {
			...
			return ::send(fd, send_buffer.data(), send_buffer.size(), 0) - sizeof(uint32_t);
	...
	ReceiveResult receive(int fd) const override {
		...
		while (true) {
			...
			if (bytes > 0) {
				...
				// Соединение закрыто
				return {ReceiveStatus::CLOSED, buffer, 0};  // Возвращаем то, что успели прочитать
			}
			else { // bytes == -1
				if (errno == EAGAIN || errno == EWOULDBLOCK) {
					// Нет доступных данных прямо сейчас
					return {ReceiveStatus::WOULDBLOCK, buffer, 0};  // Возвращаем уже прочитанное
				} else {
					// Ошибка
