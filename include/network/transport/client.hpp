...
class TCPClient : public IClient, public std::enable_shared_from_this<TCPClient> {
    ...
    TCPClient() : event_loop(std::make_unique<EventLoop>()), 
...
public:
	...
	void set_data_callback(std::function<void(const std::vector<uint8_t>&)> f) override {
		on_data_callback = f;
	}

    bool connect(const std::string& server_ip, int port) override {
        handler->init(shared_from_this());
        if (socket_visitor.create_socket() == -1) return false;
        if (socket_visitor.setup_connection(server_ip, port) < 0) return false;
        ...
        if (result.status == ReceiveStatus::OK && !result.data.empty()) {
			...
			while (framer->has_message_in_buffer(recv_buffer)) {
				...
				*/
                ...
                // std::string message(recv_buffer.begin() + sizeof(uint32_t), recv_buffer.begin() + sizeof(uint32_t) + len);
    ...
    void run_event_loop() override {
        ...
        while (isConnected()) {
            ...
            // Для тестов это ок, но сообщения от сервера будут видны только ПОСЛЕ ввода
        }
    }

    void on_writable(int fd) override {}
    void on_disconnected() override { std::cout << "Соединение разорвано" << std::endl; }
