...
public:
	...
	static std::shared_ptr<TCPServer> create() {
		// Вспомогательный класс для доступа к приватному конструктору
		struct make_shared_enabler : public TCPServer {};
	...
	void start(int port) override {
		...
		if (socket_visitor.get_fd() != -1) {
			...
			event_loop->add_fd(socket_visitor.get_fd(), acceptHandler, EPOLLIN | EPOLLOUT | EPOLLERR | EPOLLHUP | EPOLLRDHUP);
	...
	void on_client_connected(std::shared_ptr<ClientConnection> conn) override {
		...
		event_loop->add_fd(fd, handler, EPOLLIN | EPOLLOUT | EPOLLERR | EPOLLHUP | EPOLLRDHUP);
	}
