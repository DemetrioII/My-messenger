#include "../../../include/services/message_dispatchers/server_file_handlers.hpp"
...
void ServerFileStartHandler::handleMessage(const Message &msg,
  ...
  if (!recipient_fd_res.has_value()) {
    context->transport_server->send(context->fd, StaticResponses::USER_NOT_FOUND);
    return;
  }
  context->transport_server->send(*recipient_fd_res, context->serializer.serialize(msg));
}
...
void ServerFileChunkHandler::handleMessage(const Message &msg,
                                           std::shared_ptr<ServerContext> context) {
  auto recipient_username = recipient_from(msg);
  auto recipient_fd_res = context->session_manager->get_fd(recipient_username);
  if (!recipient_fd_res.has_value()) {
...
void ServerFileEndHandler::handleMessage(const Message &msg,
                                         std::shared_ptr<ServerContext> context) {
  auto recipient_username = recipient_from(msg);
  auto recipient_fd_res = context->session_manager->get_fd(recipient_username);
  if (!recipient_fd_res.has_value()) {
